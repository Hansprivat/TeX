...
% DER HENVISES TIL OG KOPIERES FLITTIGT FRA BMS. DERFOR SKAL DER VÆRE KONSISTENS!
...
\subsubsection*{This document revised 2002.10.23}
...
Open the file Log.dat in the directory LogError. Here you will
find an indication if suspicious things were discovered. If this
is the case, the contents of the file  will instruct you on how
find more details. Observe that the mentioned file may not be
updated if the GAMS system reported errors, therefore always
check the date and time in the file.

Remember:
\begin{itemize}
\item After any model run which was somehow declared successful
by GAMS, check the file Log.dat.
\end{itemize}
...
Open one file with the extension 'out' in the   directory Printout
and study the contents. You may also study the contents of other
files  with the extension 'out' in the same directory.
...
Open the file DE.INC in the directory Model. For the country and
the year simulated (search one or more of the output files, cf.
Section \ref{BGS-SS-Outputfiles}, to identify this) change the
electricity demand in  one of the regions (see \BMS for the
definition of region).  Rerun the model and study the
consequences for the output in various files (Section
...
the advise given in Section \ref{BGS-SS-LogandErrorfiles}!
...
simulated. The file is useful for getting an overview of the
model and data, for error finding and otherwise.

The simulated model need not represent all the countries and all
the years that are indicated in the include files (eg. DE.INC, cf.
Section \ref{BGS-SS-ChangeDemand}).
 %!!!ZZZ!!!
...
asterix (*) in the first position of a line turns that line into
a comment. Only one country from the set CCC is selected for
...
data structure are given as SET SSS and the seasons  simulated
are given in SET S(SSS), both located in the file SETS.INC.

The subdivision of the seasons (e.g., 4-hours intervals)  are
given in the data structure by  SET TTT and the ones simulated
are given in SET T(TTT), both located in the file SETS.INC.
...
and hence what should be changed in order to  eliminate them.
 ...
 \item Limited output: files PRINT1.INC,   PRINT2.INC,   PRINT3.INC,   PRINT4.INC,
...
specifications. These files are located in the subdirectory
 %!!!ZZZ!!!
 PRINTINC and have the extension '.INC'. The file PRINT4.INC
specifies which files that are selected.

In PRINT4.INC you may comment in some more print files (provided
of course that they exist and are properly located,  defined and
specified, note in particular PRINT1.INC). Comment in two or
three more files, run the model and observe the new files in the
subdirectory
 %!!!ZZZ!!!
...
Place the statement
 %!!!ZZZ!!!
 "DISPLAY 'My Own Choice: ', Y, C, G, DE, VGE\_T.L , 'End My Own
Choice' ;" at the very end of the file Balmorel.gms. Run the
model and find  the result in Balmorel.lst (near the end of the
file).

Various options control the type, amount and format of the output
in the file Balmorel.lst. A short description is given in the
first approximate 60 lines of the file
 %!!!ZZZ!!!
 Balmorel.gms.  See further the GAMS User's Manual (Section
\ref{BGS-S-GAMS}).
...
Since the Excel user interface is under construction it will no
be explained here.
...
started. To proceed, you will will have to experiment yourself.
You may benefit from studying other documents mentioned in Section
